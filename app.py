    ...
    def metric_cards(items, cols=4):
        ...
        for label, value, delta in items:
            if delta:
                ...
                dp = Paragraph("", ST_LBL)
            # Each cell is a nested mini-table: [value+delta] / [label]
            inner = Table(
...
with st.expander("⚙️  Data Source & Model", expanded=True):
    ...
    with _col_model:
        ...
        # ── Try to auto-load any .pkl in the app directory ──
        _auto_pkl = None
        ...
        # ── Allow user to upload a model file ──
...
with nav_tab3:
    ...
    if model_bytes is None:
        st.warning(
            ...
            "**⚙️ Data Source & Model** panel above to enable predictions."
        )
        st.stop()
